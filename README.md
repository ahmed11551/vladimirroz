...
## Настройка Telegram Bot (@Rozygvlad_bot)

1. Задеплойте фронт на Vercel и получите URL (например `https://vladimirroz.vercel.app`)
2. Настройте кнопку меню бота (токен не коммитить!):
```bash
BOT_TOKEN=ваш_токен MINI_APP_URL=https://vladimirroz.vercel.app npm run bot:setup
```
3. Откройте [@Rozygvlad_bot](https://t.me/Rozygvlad_bot) и нажмите кнопку меню (слева внизу) «🎁 Открыть розыгрыши»
